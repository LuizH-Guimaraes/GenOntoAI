...
import {
  RelationshipEntity,
  parseSVOListToRelationships,
} from "@/app/_models/relationship";

type RelationshipMapProps = {
  open: boolean;
  setOpen: Dispatch<SetStateAction<boolean>>;
  svoList: string[];
  insertList: (relationships: RelationshipEntity[]) => void;
};
...
export function RelationshipMap({
  ...
  const [relationshipList, setRelationshipList] = useState<
    RelationshipEntity[]
  >(parseSVOListToRelationships(svoList));
  ...
  function deleteRelationship(idToDelete: number) {
    setRelationshipList((prev) => {
      if (!prev) return [];

      // Remove o item com o ID
      const filtered = prev.filter((rel) => rel.id !== idToDelete);

      // Reindexa os IDs
      const reindexed = filtered.map((rel, index) => ({
        ...rel,
        id: index,
      }));
  ...
  function handleSubmit() {
    insertList(relationshipList);
  }

  function updateList(relationship: RelationshipEntity, index: number) {
    ...
    <SlideOver isOpen={open} onClose={() => setOpen(false)} side="right">
      ...
      <div className="flex flex-col h-[calc(100vh-90px)]">
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 bg-white flex flex-col gap-3">
          ...
          <button
            className="w-full py-2 px-4 cursor-pointer bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition"
            onClick={() => {
              handleSubmit();
            }}
          >
